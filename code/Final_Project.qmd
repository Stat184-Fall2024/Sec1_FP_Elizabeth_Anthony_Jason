...
1. Obvious--- recommended by teachers, internet resources, and its benefits observed throughout one's precollege studies--- and therefore our research would be redundant, and 
...
Second, we will look at the impact of absences on GPA as well. At face-level, we think that absences will also negatively correlate with GPA, as we think that going to class is interrelated to succeeding academically.
...
```{r}
#| echo: false
#| fig-cap: Figure 2.3\:\ Effect of Absences on GPA
...
# Creating a "Geom Point" grid excluding the few un-supported points
ggplot(
  data = abs_alc_ct, 
  mapping = aes(
      x = absences_average, 
